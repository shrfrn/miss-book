...
function query(){
    return storageService.query(MISS_BOOK)
}
...
function remove(bookId){
    storageService.remove(MISS_BOOK, bookId)
}

function save(book){
    if(book.id) {
        storageService.put(MISS_BOOK, book)
      } else {
        storageService.post(MISS_BOOK, book)
      }
}
...
function _createBooks() {
    let books = utilService.loadFromStorage(MISS_BOOK);
    if (!books || !books.length) {
        books = [];
        books.push(..._books);
        utilService.saveToStorage(MISS_BOOK, books);
    }
    return books;
}
...
[
    {
      ...
      "authors": [
        "Barbara Cartland", "Sharon Frenkel", "Baba Ganush"
      ],
      "publishedDate": 2021,
